...
export class PostsService {
  ...
  async create(user: any, createPostDto: CreatePostDto): Promise<Post> {
    ...
    try {
      ...
      await this.auditLogsService.createLog(
        'post_created',
        `Post "${saved.title}" created by ${user.email}`,
        {
      ...
      if (error.code === 11000) {
        throw new BadRequestException(
          'Judul post sudah digunakan di workspace ini',
        );
      }
      throw error;
    }
  ...
  async update(
    ...
    try {
      const updatedPost = await this.postModel
        .findByIdAndUpdate(
          id,
          { ...updatePostDto, ...additionalUpdates },
          { new: true },
        )
        .exec();
      ...
      await this.auditLogsService.createLog(
        'post_updated',
        `Post "${updatedPost?.title}" updated${userText}`,
        { postId: id, workspaceId: updatedPost?.workspaceId.toString() },
      );
  ...
  async delete(id: string, user?: any): Promise<void> {
    ...
    await this.auditLogsService.createLog(
      'post_deleted',
      `Post "${post.title}" deleted${userText}`,
  ...
  async incrementRetryCount(id: string): Promise<void> {
    await this.postModel
      .findByIdAndUpdate(id, { $inc: { retryCount: 1 } })
      .exec();
  }
  ...
  async cancelSchedule(id: string, user?: any): Promise<void> {
    ...
    const userText = user ? ` by ${user.email}` : '';
    await this.auditLogsService.createLog(
      'post_schedule_cancelled',
      `Schedule for post "${post.title}" cancelled${userText}`,
      { postId: id, workspaceId: post.workspaceId.toString() },
    );
  ...
  async publishNow(id: string, user?: any): Promise<void> {
    ...
    await this.auditLogsService.createLog(
      'post_publish_now',
      `Post "${post.title}" submitted for immediate publication${userText}`,
      { postId: id, workspaceId: post.workspaceId.toString() },
    );
  }
  ...
  async handleCronAnalyticsSync() {
    ...
    const publishedPosts = await this.postModel
